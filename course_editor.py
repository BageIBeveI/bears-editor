...
colcolPixel = 40
smalltilePixel = 4
numOfTinySubtilesVt = ((320+smalltilePixel) // (smalltilePixel * 8))
thingyPixel = 40
paletteDataOffset = 0
...
INFO_IMAGES = [pygame.image.load(f'tile_editor_pictures/infoCourseEditor.png'), pygame.image.load(f'tile_editor_pictures/infoTileEditor.png'), pygame.image.load(f'tile_editor_pictures/infoSpriteEditor.png')]
...
EYE_IMAGES = [pygame.image.load(f'tile_editor_pictures/eyesEeNothing.png'), pygame.image.load(f'tile_editor_pictures/eyesWideOpen.png')]
...
def bmpPrintrer17(bigtile, layaer):
    ...
    for j in (range(spriteHeight[layaer])[::-1]):  # first j will add the bottom subtiles to guaug, second j will add the top ones
        ...
        for k in range(8):
            for l in range(spriteWidth[layaer]):
                for bit in graphicsies[l][k]:  # graphicsies[0] has 8 bytes in its list (contains the left subtile's data.) ([1] has 8 bytes for the right subtile)
                    guaug.append(palettecode[int(bit) + 4 * l])  # would += to a b'' but that takes super long
...
def tinySubtileDraw(subGraphics, hexCodes, palSelected, scroll):
    for k in range(32): #max 512 tiles, so 32 high
        for l in range(16): #16 wide
            for i in range(8): #and subtiles are 8x8px, handled by i and j
                ...
                for j in range(8):
                    ...
                    try:
                        colourSelected = int(subGraphics[((k * 16) + l) * 8 + i][j])
                    ...
                    if loadedSpriteType == 17:
                        ...
                        palselels = spritePalettes[layerChosen][(l)+(k*16)]
        ...
        for j in range(spriteWidth[layaaaa]):
            ...
            for k, K in enumerate(vt_range):
                for l, L in enumerate(hz_range):
                    rectangle = pygame.Rect = ((l * quadPixel + j * quadPixel * 8), (k * quadPixel + i * quadPixel * 8 + 50), quadPixel, quadPixel)
                    #try:
                    if loadedSpriteType == 17:
...
while running:
            ...
            if filename != "":
                ...
                if fileFound:
                    try:
                        ...
                        liszt = [int(byte) for byte in tempArrrgh[3:-1]] #the -1 takes off the last \n of a csv file, and the 16 is to remove the empty header junk
    ...
    #
        ...
        if scrollLeft or scrollRight:
            ...
            if scrollRight == True:
                bigtileSelected += 1
            pygame.time.wait(20//(scrollSpeed**2))
        ...
        if bigQuadMovement != 0:
            match bigQuadMovement:
                ...
                case 4:
                    ...
                    if bigtileQuadrantSelected >= spriteWidth[layerChosen] * spriteHeight[layerChosen]:
                        bigtileQuadrantSelected = (spriteWidth[layerChosen] * spriteHeight[layerChosen]) - 1
            pygame.time.wait(50//scrollSpeed)
        ...
        if pygame.mouse.get_pressed()[0] == 1:
            ...
            elif subtileMode and 0 < mouseX < (quadPixel * 2 * 8) and 50 < mouseY < 50 + (quadPixel * 2 * 8) and (loadedSpriteType == 17 or subtileSelected != 0): #end part is so the ghost tile isn't editable
            ...
            elif 750 < mouseX < 750+smalltilePixel*8*16 and 50 < mouseY < 50 + smalltilePixel * 8 * (320//(smalltilePixel * 8)): #for how many things can fit, usually 10 for 320px but i want to make this a little more editable so ha !
                if subtileGraphics[layerChosen][subtileSelected*8:subtileSelected*8+8] != currentSubtileStackInfo:
                    undoStack.append([subtileSelected, currentSubtileStackInfo])
                    if len(undoStack) > 150:
                        undoStack = undoStack[50:]
                    redoStack = []
                ###print("C")
                subtileSelected = int(((mouseY - scroll2 - 50) // (smalltilePixel*8)) * 16 + ((mouseX - 750) // (smalltilePixel*8)))
        ...
        # and highlight the selected one
        Y = (smalltilePixel*8 * (subtileSelected // 16) + 50 + scroll2)
        numOfTinySubtilesVt = ((320+smalltilePixel) // (smalltilePixel * 8))
        if Y < 50:
            pygame.draw.line(SCREEN, (255, 0, 0), (750, 50), (1261, 50))
        elif Y < 50+numOfTinySubtilesVt*8*smalltilePixel: #another not *10 so ha for the 336 part
            highlightangle = (smalltilePixel*8 * (subtileSelected % 16) + 750, Y, smalltilePixel*8, smalltilePixel*8)
            pygame.draw.rect(SCREEN, (255, 0, 0), highlightangle, 1)
        else:
            pygame.draw.line(SCREEN, (255, 0, 0), (750, 50+numOfTinySubtilesVt*8*smalltilePixel), (750+smalltilePixel*8*16, 50+numOfTinySubtilesVt*8*smalltilePixel))
